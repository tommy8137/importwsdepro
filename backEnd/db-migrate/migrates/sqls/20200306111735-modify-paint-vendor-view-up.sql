...
case when (select count(link_b.id) from formula.v_plastic_painting_vendor_type_color_bottom_top link_b where link_b.paint_color_id = link.paint_color_id and link_b.paint_type_id = link.paint_type_id and link_b.paint_bottom_top_name = '底漆' and link_b.link_disable_time is null) > 0 then true else false end as bottom,
case when (select count(link_t.id) from formula.v_plastic_painting_vendor_type_color_bottom_top link_t where link_t.paint_color_id = link.paint_color_id and link_t.paint_type_id = link.paint_type_id and link_t.paint_bottom_top_name = '面漆' and link_t.link_disable_time is null) > 0 then true else false end as top,
