...
update formula.part_category_material set disable_time = now() where material_id in (select id from formula.material m where m.material_name = 'Other_Fill_ME_Remark');

update formula.part_category_material set disable_time = null where material_id in (select id from formula.material m where m.material_name = 'Other_Fill_ME_Remark' and m.material_spec_id in (select id from formula.material_spec ms where ms.material_spec_name in ('PC','TPEE','TPU', 'Other_Fill_ME_Remark')));

update formula.cablewire_connector_item set disable_time = now() where connector_item = 'Cable material price';

INSERT INTO formula.part_category_material (part_category_2_id, material_id)
select (select id from formula.part_category_2 pc where pc.category_name = 'Insert_Molding'), m.id from formula.material m where m.disable_time is null on conflict on constraint part_category_material_pk do update set disable_time = null; 

INSERT INTO formula.part_category_material (part_category_2_id, material_id)
select (select id from formula.part_category_2 pc where pc.category_name = 'Double_Injection'), m.id from formula.material m where m.disable_time is null on conflict on constraint part_category_material_pk do update set disable_time = null; 

INSERT INTO formula.part_category_formula_type (part_category_2_id, formula_type_id) VALUES((select id from formula.part_category_2 pc where pc.category_name = 'Insert_Molding'), (select id from formula.formula_type ft where ft."name" = 'housing_plastic')) on conflict do nothing;

alter table wiprocurement.col_dependence add column if not exists disable_time timestamptz null;

drop view if exists formula.v_part_category_col_definition;
CREATE OR REPLACE VIEW formula.v_part_category_col_definition
AS SELECT DISTINCT pc.id AS part_category_2_id,
...
INSERT INTO wiprocurement.bom_partlist_config (parts_ctgy_1, parts_ctgy_2, format) VALUES((select pc.part_category_1_id from formula.part_category_2 pc where pc.category_name = 'Insert_Molding'), (select id from formula.part_category_2 pc where pc.category_name = 'Insert_Molding'), (select id from wiprocurement.bom_partlist_format bpf where bpf.format_key = 'housing-plastic')) on conflict do nothing;

insert into wiprocurement.col_dependence(col_val, col_id, required_col_id) values(
        (select id from formula.part_category_2 pc where pc.category_name='Insert_Molding'), 
       (select id from  wiprocurement.col_definite where col_key='parts_ctgy_2' and used_by='bom_item'),
       (select id from wiprocurement.col_definite where used_by='bom_item' and col_name='H')) on conflict do nothing;
insert into wiprocurement.col_dependence(col_val, col_id, required_col_id) values(
        (select id from formula.part_category_2 pc where pc.category_name='Insert_Molding'), 
       (select id from  wiprocurement.col_definite where col_key='parts_ctgy_2' and used_by='bom_item'),
       (select id from wiprocurement.col_definite where used_by='bom_item' and col_name='L')) on conflict do nothing;
insert into wiprocurement.col_dependence(col_val, col_id, required_col_id) values(
        (select id from formula.part_category_2 pc where pc.category_name='Insert_Molding'), 
       (select id from  wiprocurement.col_definite where col_key='parts_ctgy_2' and used_by='bom_item'),
       (select id from wiprocurement.col_definite where used_by='bom_item' and col_name='Part Weight')) on conflict do nothing;
insert into wiprocurement.col_dependence(col_val, col_id, required_col_id) values(
        (select id from formula.part_category_2 pc where pc.category_name='Insert_Molding'), 
       (select id from  wiprocurement.col_definite where col_key='parts_ctgy_2' and used_by='bom_item'),
       (select id from wiprocurement.col_definite where used_by='bom_item' and col_name='Thickness')) on conflict do nothing;
insert into wiprocurement.col_dependence(col_val, col_id, required_col_id) values(
        (select id from formula.part_category_2 pc where pc.category_name='Insert_Molding'), 
       (select id from  wiprocurement.col_definite where col_key='parts_ctgy_2' and used_by='bom_item'),
       (select id from wiprocurement.col_definite where used_by='bom_item' and col_name='W')) on conflict do nothing;
