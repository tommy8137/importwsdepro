...
const ConnectorPrice = props => {
  const { getCableFfcConnectorPrice, putCableFfcConnectorPrice, date, connectorPriceList } = props;
  ...
  const handleSave = () => {
    const data = {
      ...
      items: differenceList.map(item => ({ id: item.id,  price: item.next.price, processTime: item.next.processTime }))
    };
    setEditMode(false);
...
const mapStateToProps = state => {
  return {
    connectorPriceList: state.cableffcCleanSheet.connectorPrice.connectorPrice,
    date: state.cableffcCleanSheet.connectorPrice.date,
  };
};

const mapDispatchToProps = {
  getCableFfcConnectorPrice: CableFfcCleanSheetActions.getCableFfcConnectorPrice,
  putCableFfcConnectorPrice: CableFfcCleanSheetActions.putCableFfcConnectorPrice,
};
