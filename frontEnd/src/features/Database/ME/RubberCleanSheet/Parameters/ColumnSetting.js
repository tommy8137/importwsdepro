...
const left = props => {
  const { isEditMode, selectedColumn } = props;
...
const right = props => {
  const { date, isEditMode, handleOnEditItem } = props;
  ...
  return [
    ...
    },
    {
      ...
      render: (val, record) => (isEditMode && _get(date, 'nextId', false) ? <NumberInput value={val} onChange={value => handleOnEditItem(value, record.id, 'next')} /> : comma(val, 8, '－')),
    },
  ];
};
