...
function MetalCleanSheet(props) {
  const [openSchedule, setOpenSchedule] = useState(false);
  const { schedule, productTypeList, activeProductType } = props;


  useEffect(() => {
    props.getProductType(Constants.FORMULA_TYPE.metal);
  }, []);

  useEffect(() => {
    if (openSchedule) {
      props.getSchedule(Constants.FORMULA_TYPE.metal);
    }
  }, [openSchedule]);
  ...
  return (
    <Fragment>
      ...
      {Content.map(item => {
        const isValid = (btn) => {
          return btn.routerName !== '' && activeProductType && btn.validProductTypeID.includes(Number(activeProductType.value));
        };
        const validItems = item.btns.filter(isValid);
        return (
          <div className="groupBox" key={item.group}>
            <div className="groupTitle">
              {(validItems && validItems.length > 0) && item.group}
            </div>
            {item.btns.map(btn => {
              return isValid(btn) && (
                <Btns
