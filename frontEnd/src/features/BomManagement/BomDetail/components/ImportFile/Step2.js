...
import { Step2Div, ErrorBox } from './ImportFileStyles';


function Step2(props) {
  const { passCount, failCount, failRows } = props;
  return (
    <Fragment>
      {/* 成功及失敗資料筆數 */}
      <Step2Div>
      ...
      <ErrorBox>
        ...
        <div className="error-detail">
          ...
          {failRows.map((row, index) => (<p className="error-detail-item" key={index}>Row {row.item} {getMessageByErrorCode(row.errorCode)}</p>))}
        </div>
      </ErrorBox>
    </Fragment>
  );
