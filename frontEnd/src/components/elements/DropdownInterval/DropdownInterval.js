...
const DropdownInterval = (props) => {
  ...
  return (
    <DropdownBox
      disabled={disabled}
      className="dropdown e2e_dropdown_interval"
    ...
    >
      ...
      >
        <RangeDiv>
          <p>搜尋結果之最低價 至 最高價</p>
          <p>${round(inputMin)} ~ {round(inputMax)}</p>
          <hr />
          <p>篩選範圍</p>
          ...
          <Button color="black" border={false} className="submit" onClick={onSubmit}>套用</Button>
        </RangeDiv>
      </DropdownMenu>
    </DropdownBox>
